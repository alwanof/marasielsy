          ...
          <tbody>
            <tr v-for="(driver, index) in drivers.data" :key="driver.id">
              <td width="15%" v-show="acl.makeDriversApproved">
                <button
                  type="button"
                  @click="active(index,driver)"
                  :class="'btn btn-sm '+((driver.active==1)?'btn-success':'btn-warning')+' approveBtn text-light'"
                  :disabled="(driver.active==1)?true:false"
                >{{ local[lang+".leads"]["approve"] }}</button>
              </td>
              <td>
                <img :src="driver.avatar" class="rounded-circle" width="42" />
              </td>
              <td>{{ driver.fname }}</td>
              <td>{{driver.lname}}</td>
              <td>
                ...
                <span class="badge badge-secondary">
                  {{(driver.vtype=='0')?local[lang+".leads"]["vtypelist"][0]:local[lang+".leads"]["vtypelist"][0]
                  }}
                </span>
              ...
              <td>
                <span v-for="attach in driver.attachs">
                  <a
                    :href="path+'/modules/drivers/uploads/attach/'+driver.email+'/'+attach"
                    target="_blank"
                  >{{attach}}</a>
                </span>
              </td>
...
export default {
  ...
  methods: {
    ...
    active(index, driver) {
      ...
      const user = CONFIG.dbAuth
        .createUserWithEmailAndPassword(driver.email, "secret+9314")
        .then(doc => {
          if (doc) {
